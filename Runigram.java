...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0 ; i < flipped.length ; i ++) {
			for ( int j = 0 ; j < flipped[0].length ; j ++){
				flipped [i][flipped[0].length - j - 1] = image[i][j];
			}
		}
		return flipped;
	}
	...
	public static Color luminance(Color pixel) {
		...
		Color lum = new Color(value,value,value);
		return lum;
	}
	/**
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		int origin_w = image[0].length;
		int origin_h= image.length;

		for (int i = 0 ; i < height ; i ++) {
			for (int j = 0 ; j < width ; j ++){
				int result_i = i * origin_h / height;
				int result_j = j * origin_w / width;
				scaled[i][j] = image[result_i][result_j];
			}
		}
		return scaled ;
	}
